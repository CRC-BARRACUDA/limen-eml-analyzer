...
#   dist/eml_analyzer-<os>-<arch>.<ext>          the prebuilt library (the release asset)
#   dist/eml_analyzer-<os>-<arch>.<ext>.sha256   its checksum (required by the manager)
...
name="eml_analyzer"   # must match [lib] name in Cargo.toml (and `entry` in limen.toml)
